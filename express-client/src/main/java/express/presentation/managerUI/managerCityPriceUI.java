...
public class managerCityPriceUI extends JPanel {
	...
	private boolean checkVectorData(Vector<JTextField> vectf) {
		...
		if (!right) {
			JOptionPane.showMessageDialog(null, "距离应填写小数", "提示",
					JOptionPane.ERROR_MESSAGE);
		}
		return right;
	}
	...
	private boolean checkTextFieldFull(JTextField tf) {
		...
		if (tf.getText().isEmpty()) {
			...
			JOptionPane.showMessageDialog(null, "城市名未填写", "提示",
					JOptionPane.ERROR_MESSAGE);
		}
	...
	private boolean isCityExist(String city, boolean add) {
		...
		if (add) {
			if (row != -1) {
				JOptionPane.showMessageDialog(null, "该城市已存在", "提示",
						JOptionPane.ERROR_MESSAGE);
			}else{
				exist = false;
			}
			...
			if (row == -1) {
				exist = false;
				JOptionPane.showMessageDialog(null, "该城市不存在", "提示",
						JOptionPane.ERROR_MESSAGE);
			} else {
				exist = true;
				changerow = row;
			...
			if (e.getSource() == priceadd) {
				...
				if (complete) {
					...
					if (right) {
						...
						JOptionPane.showMessageDialog(null, "设置成功", "提示",
								JOptionPane.INFORMATION_MESSAGE);
					} else {
						JOptionPane.showMessageDialog(null, "价格应为小数", "提示",
								JOptionPane.WARNING_MESSAGE);
					}
				} else {
					JOptionPane.showMessageDialog(null, "价格未全部填写", "提示",
							JOptionPane.ERROR_MESSAGE);
				}
